...
import { formatToLocalISO } from "@/lib/utils";
...
export function Invoice({ user }: { user: UserData }) {
  ...
  const handleGenerateQRCode = async () => {
    ...
    console.log(new Date(formatToLocalISO(invoiceDate)), 'ki ailo')
    try {
      const qrData = generateInvoiceQRCode({
        companyName: user.companyName!,
        vatNumber: user.vatNumber!,
        invoiceDate: new Date(formatToLocalISO(invoiceDate)),
        totalWithVAT: total,
        vatAmount: tax,
      });
                  ...
                  <FormItem>
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        if (selectedDate) {
                        // Use the Date object methods to avoid string parsing bugs
                        const dateWithTime = new Date(selectedDate); 
                        const now = new Date();

                        dateWithTime.setHours(now.getHours(), now.getMinutes(), 0, 0);

                        field.onChange(dateWithTime.toISOString());
                        } else {
                          field.onChange("");
                        }
                      }}
